...
| **4단계** | 테이블 완성 (계정 본편, 5e 라이트 시트·이니셔티브, 수동 안개, WebRTC 음성 + coturn) | ✅ 완료 (WebRTC 음성은 best-effort — 시그널링·coturn·자동 테스트까지 끝났지만 실기기 마이크 왕복은 미검증) |
| **5단계** | 스튜디오 v1 (문서 우선 에디터 · 발행 파이프라인) | 🟡 설계 승인 완료, 구현 시작 — [docs/STAGE5.md](./docs/STAGE5.md) |
...
### 4단계 §4 — WebRTC 음성 메시 + coturn (best-effort)

- 새 시그널링 op 3종(`voice.offer`·`voice.answer`·`voice.ice`)을 기존 테이블 WS 채널에
  얹었다 — 새 서버 없이 순수 릴레이(서버는 SDP/ICE 내용을 해석하지 않고 지정한 상대
  소켓에만 전달, `RoomState`를 안 건드리니 seq도 안 씀). 오디오 자체는 P2P mesh(≤6인).
- `GET /api/tables/:id/turn-credentials`가 coturn의 `use-auth-secret` 방식으로 단기 TURN
  자격증명을 발급한다(HMAC-SHA1, DB에 사용자별 TURN 계정 없음). `TURN_SECRET`이 없으면
  STUN만 돌려주고 coturn 없이도 동작 — coturn은 안전망이지 필수 인프라가 아니다.
  `docker-compose.yml`에 `coturn` 서비스 추가(`network_mode: host` — 릴레이 포트 광고 문제
  때문).
- 클라이언트(`apps/web/src/voice/useVoice.ts`)는 닉네임 사전순 규칙으로 offer 발신자를
  정해 glare(양쪽 동시 offer)를 피하고, 말하는 사람의 토큰에 촛불 글로우를 씌운다(§6
  시그니처) — `AnalyserNode`로 음량을 폴링해 판단.
- **best-effort로 마무리**: 시그널링 릴레이·TURN 자격증명·mute·에러 처리는 자동 테스트와
  브라우저로 확인했지만, 실제 마이크 음성 왕복은 이 자동화 환경이 Chrome의 네이티브 마이크
  권한 프롬프트를 클릭할 수 없어 검증하지 못했다 — PROMPT-stage4.md §4가 애초에 이 지점을
  best-effort로 허용해뒀다. 실기기 확인 절차는 [docs/STAGE4.md](./docs/STAGE4.md)의 "§4
  하지 못한 것"에 정리돼 있다.

### 검증 상태

- `pnpm test` — **134개 테스트 전부 green** (schema 20[R7 포함] + runtime 8 + server
  69[REST/WS e2e·주사위 파서·계정 인증·5e 시트/이니셔티브·안개·음성 시그널링·TURN 자격증명
  테스트 포함] + web 37).
...
- **4단계 — 테이블 완성**: 네 항목(계정 본편·5e 라이트 시트/이니셔티브/HP·상태·수동 안개·
  WebRTC 음성) 전부 **완료**(`pnpm test` 134개 green — [docs/STAGE4.md](./docs/STAGE4.md)
  참고). WebRTC 음성만 best-effort로, 실기기 두 대로 실제 마이크 왕복·coturn 릴레이를
  확인하는 절차가 `docs/STAGE4.md`의 "§4 하지 못한 것"에 숙제로 남아있다(테이블 자체는
  음성 없이도 완전히 돌아간다).
- 프리셋 24종 토큰 스프라이트(현재는 이니셜+색 링뿐) — v1.0 몫.
- **v0.5 나머지**: 문서 우선 에디터(슬래시 블록, [`PROMPT-stage5.md`](./PROMPT-stage5.md)에
  설계 정리됨), 홈 라이브러리 4줄 구성, 자동 포스터 고도화.
