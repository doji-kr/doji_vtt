...
- `pnpm test` — **86개 테스트 전부 green** (schema 20[R7 포함] + runtime 8 + server 31[REST/WS
  e2e·주사위 파서 단위 테스트 포함] + web 27).
...
- **4단계 — 테이블 완성**: WebRTC 음성 메시 + coturn, 수동 안개, 5e 라이트 시트·이니셔티브·
  HP/상태, 계정 시스템 본편(argon2id)과 이중 프로필의 시작, 프리셋 24종 토큰 스프라이트.
