...
| R7 choice-softlock | warn | 도달 가능한 choice 블록의 모든 옵션이 `requires_flag` 조건부 — 플래그 조합에 따라 선택지가 0개가 될 수 있음 |

R7이 error가 아니라 warn인 이유: 그 블록에 도달하는 시점에 항상 참인 플래그가 있을 수도
있어서 정적으로는 "반드시 소프트락"이라고 확신할 수 없다. 런타임(서버)은 이 소프트락을
`showChoices` effect의 `options.length === 0`을 감지해 409로 이미 막고 있다 — R7은 그 상황이
저작 시점에 미리 보이게 하는 조기 경보다.
