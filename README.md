...
| **4단계** | 테이블 완성 (계정 본편, 5e 라이트 시트·이니셔티브, 수동 안개, WebRTC 음성 + coturn) | 🟡 진행 중 (계정 본편 완료, 나머지 3개 남음) |
...
- 작업 기록·판단 콜·검증 절차는 [docs/STAGE4.md](./docs/STAGE4.md), 다음 단계(5e 라이트
  시트·수동 안개·WebRTC 음성)로 이어받는 방법도 그 문서에 정리돼 있다.

### 검증 상태

- `pnpm test` — **101개 테스트 전부 green** (schema 20[R7 포함] + runtime 8 + server
  43[REST/WS e2e·주사위 파서·계정 인증 단위 테스트 포함] + web 30).
...
- **4단계 — 테이블 완성**: 계정 시스템 본편(argon2id, 게스트 입장과 공존)과 이중 프로필의
  시작은 **완료**(`pnpm test` 101개 green — [docs/STAGE4.md](./docs/STAGE4.md) 참고). 남은
  세 항목: 5e 라이트 시트·이니셔티브·HP/상태, 수동 안개(브러시), WebRTC 음성 메시 + coturn.
  진행 방식·순서·이어받는 방법은 [`PROMPT-stage4.md`](./PROMPT-stage4.md)와
  `docs/STAGE4.md`의 "재개 방법" 절에 정리돼 있다.
