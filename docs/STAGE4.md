# 4단계 작업 기록 — §1 계정 본편 (최소 침습 마이그레이션)

## 재개 방법 (다음 세션에서 이어가기)

**지금 상태**: §1(계정 본편)만 끝났다. `pnpm test` 101개 전부 green, 커밋·푸시 완료
(`master` 최신). §2(5e 라이트 시트·이니셔티브·HP/상태)부터 이어가면 된다.

새 세션에서:
1. `PROMPT-stage4.md`를 읽게 시키되, **§1은 이미 끝났으니 건너뛰고 §2("5e 라이트 시트 ·
   이니셔티브 · HP/상태")부터 시작**하라고 알려준다 — 이 문서(`docs/STAGE4.md`)의 "아키텍처
   결정"·"판단 콜" 절을 먼저 읽게 해서 계정 계층이 어떻게 생겼는지(회원 `userId` vs 게스트
   `guestName`, 소유권은 반드시 `userId`로만 판단) 파악시킨 다음 진행한다.
2. §2 작업은 `characters` 테이블을 새로 만들 텐데, PROMPT-stage4.md가 이미
   `owner_user_id NOT NULL`로 못 박아뒀다 — 게스트는 캐릭터 시트를 못 만든다(이번 §1에서
   깔아둔 회원/게스트 구분을 그대로 재사용하면 된다, 새로 설계할 것 없음).
3. 순서는 여전히 "계정 → 5e 시트 → 안개 → 음성" 이다 — §2 끝나면 §3(수동 안개), 그다음
   §4(WebRTC 음성, best-effort)로 이어간다.
