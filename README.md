...
| **3단계** | 라이브 테이블 코어 (WS 이벤트 릴레이, PixiJS 지도·토큰, 실시간 주사위·채팅, R7 린터) | ⬜ 예정 |
| **4단계** | 테이블 완성 (WebRTC 음성 + coturn, 수동 안개, 5e 라이트 시트·이니셔티브, 계정 본편) | ⬜ 예정 |
...
- **채널 분리 증명 테스트**: 실패 경로까지 포함한 전체 플레이 동안 어떤 API 응답에도
  `dm_notes`/`Npc.secret` 문자열이 등장하지 않음을 자동 검사.
- 두 엔딩("봉인, 다시" / "평범한 하루") 모두 CLI·API·브라우저 각 경로에서 완주 확인.
- 클린 클론 기준 제3자 재현 검증 완료(테스트 스위트 + 라이브 API 스모크).

## 남은 것

- **3단계 — 라이브 테이블 코어**: WebSocket 이벤트 릴레이(서버 권위·seq·재접속 스냅샷),
  PixiJS 지도·그리드·토큰 동기화, 서버 굴림 주사위와 통합 모험 일지, R7 린터 규칙
  (빈 choice 소프트락의 저작 시점 정적 감지 — 지금은 서버 409 가드로만 방어 중).
- **4단계 — 테이블 완성**: WebRTC 음성 메시 + coturn, 수동 안개, 5e 라이트 시트·이니셔티브·
  HP/상태, 계정 시스템 본편(argon2id)과 이중 프로필의 시작.
