# 4단계 작업 기록 — §1 계정 본편 + §2 5e 라이트 시트·이니셔티브·HP/상태 + §3 수동 안개

## 재개 방법 (다음 세션에서 이어가기)

**지금 상태**: §1(계정 본편)·§2(5e 라이트 시트·이니셔티브·HP/상태)·§3(수동 안개) 끝났다.
`pnpm test` 120개 전부 green. §4(WebRTC 음성 + coturn, best-effort)만 남았다.

새 세션에서:
1. `PROMPT-stage4.md`를 읽게 시키되, **§1·§2·§3은 이미 끝났으니 건너뛰고 §4("WebRTC 음성
   메시 + coturn")부터 시작**하라고 알려준다 — 이 문서의 "§1/§2/§3 아키텍처 결정" 절을 먼저
   읽혀서 회원/게스트 구분, 캐릭터·이니셔티브·안개 실시간 op가 어떻게 생겼는지 파악시킨 다음
   진행한다.
2. §4는 새 시그널링 op 3종(`voice.offer`/`voice.answer`/`voice.ice`)을 기존 테이블 WS
   채널에 얹는 작업이다 — §3에서 `fog`를 `RoomState`에 추가하고 `LiveRoom`에 엮은 패턴
   (`apps/server/src/room-registry.ts`)과 달리, 시그널링은 방 상태(`RoomState`)에 남기지
   않는 순수 릴레이라는 점이 다르다(서버는 SDP/ICE 내용을 해석하지 않고 대상 nickname/userId로
   전달만 한다). `docker/docker-compose.yml`에 `coturn` 컨테이너 추가와 TURN 자격증명 발급
   (HMAC 공유 비밀, `use-auth-secret`)도 §4 범위다.
3. §4는 best-effort다 — 시간이 부족하면 여기서 멈춰도 나머지 세 항목(계정·5e 시트·안개)은
   온전히 쓸 수 있다. README 로드맵 4단계 행은 §4 완료 여부와 무관하게 이번 세션에서 갱신했다
   (WebRTC 음성만 "남음"으로 정직하게 표기).
